...
int main()
{
    Stack stack;        //Stack 클래스 객체 stack 선언
    ...
    while (1)           //무한반복 시작
    {
        ...
        if (!strcmp(cmd, "size")) {     //명령어가 size라면
            int size;
            cin >> size;                //size입력 받기
            stack.SetSize(size);        //입력받은 size만큼 stack의 사이즈를 조절
        }
            ...
            if (stack.Push(temp)) {         //해당 노드를 push하기
            ...
            if (stack.IsEmpty() == true) {     //스택이 비어있다면
                cout << "스택이 비어있음" << endl; //비어있음 출력
            }
            else {                          //아니라면
                Node* temp = stack.Pop();      //pop함수 실행해 temp에 저장
            ...
            stack.PrintStack();         //비어있지 않다면 스택 안에 데이터를 출력하는 함수 실행
        }
